...
namespace SnowyPeak.Duality.Plugin.Frozen.UI.Widgets
{
	...
	public abstract class Widget : Component, ICmpRenderer, ICmpUpdatable, ICmpInitializable
	{
		...
		private string _customWidgetAppearance;
		private Widget _next, _previous;

		private Rect _rect, _clipRect;

		private ContentRef<Skin> _skin;
		...
		public Widget()
		{
			...
			Skin = ContentProvider.RequestContent<Skin>(FrozenUIPlugin.DEFAULT_SKIN);
		}
...
#pragma warning restore 1591
		...
		/// </summary>
		public string CustomWidgetAppearance
		{
			get { return _customWidgetAppearance; }
			set
			{
				_customWidgetAppearance = value;
		...
		{
			if (_skin.IsAvailable)
			{
				if (_skin.Res.WidgetAppearances.ContainsKey(GetAppearanceName()))
				{
		...
		protected virtual void DrawVertices(IDrawDevice device)
		{
			...
			_skin.Res.Clip(ref _vertices, _rect, _clipRect, GetAppearanceName());
			_skin.Res.Draw(device, _vertices);
		}
		/// <summary>
		/// Returns the apperance name for the Widget - either the Object name or, if set, CustomWidgetAppearance
		/// </summary>
		/// <returns></returns>
		protected string GetAppearanceName()
		{
			return String.IsNullOrEmpty(CustomWidgetAppearance) ? this.GameObj.Name : CustomWidgetAppearance;
		}
		...
		protected virtual void PrepareVertices(IDrawDevice inDevice)
		{
			 ...
			 * 12   13   14   15
			 ********************/
			_skin.Res.PrepareVertices(ref _points, _rect, GameObj.Transform.Scale, GetAppearanceName(), Status);
